...
const nodeHTTP__default = /*#__PURE__*/_interopDefaultCompat(nodeHTTP);
const nodeHTTPS__default = /*#__PURE__*/_interopDefaultCompat(nodeHTTPS);
...
const nativeAgents = { http: nodeHTTP__default, https: nodeHTTPS__default };
...
const stream = defineProxyMiddleware(
  (req, socket, options, server, head, callback) => {
    ...
    const proxyReq = (isSSL.test(options.target.protocol || "http") ? nodeHTTPS__default : nodeHTTP__default).request(
      setupOutgoing(options.ssl || {}, options, req)
    );
    if (server) {
...
class ProxyServer extends node_events.EventEmitter {
  ...
  listen(port, hostname) {
    ...
    this._server = this.options.ssl ? nodeHTTPS__default.createServer(this.options.ssl, closure) : nodeHTTP__default.createServer(closure);
    if (this.options.ws) {
...
async function proxyFetch(addr, input, inputInit) {
  ...
  const res = await new Promise((resolve, reject) => {
    ...
    const req = nodeHTTP.request(reqOpts, resolve);
    req.on("error", reject);
    if (init.body instanceof ReadableStream) {
      const readable = node_stream.Readable.fromWeb(init.body);
  ...
  return new Response(hasBody ? node_stream.Readable.toWeb(res) : null, {
...
function proxyUpgrade(addr, req, socket, head, opts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const doRequest = isSSL.test(target.protocol) ? nodeHTTPS.request : nodeHTTP.request;
...
exports.proxyUpgrade = proxyUpgrade;
