...
function justName(filename = "") {
  return stripExtension(node_path.basename(filename));
}
function stripExtension(filename) {
  return filename.replace(node_path.extname(filename), "");
}
...
rules$1.set("Transform definitions to $defs", (schema, fileName) => {
  if (schema.definitions && schema.$defs && !node_util.isDeepStrictEqual(schema.definitions, schema.$defs)) {
    throw new ReferenceError(
    ...
    case "UNION":
      return {
        ...
        params: schema.type.map((type2) => {
          const member = { ...omit(schema, "$id", "description", "title"), type: type2 };
    ...
    case "UNTYPED_ARRAY": {
      ...
      if (minItems > 0 || maxItems >= 0) {
        return {
          ...
          params: Array.from({ length: Math.max(maxItems, minItems) || 0 }).fill(params),
